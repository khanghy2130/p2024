...
const scrollProgressRange = parseInt(
...
function updateScroll() {
  ...
  if (oldGreetProgress !== greetProgress) {
    ...
    frontDiv.style.clipPath = `polygon(0 0, 0 100%, ${
      displayProgress - tiltPercent
    }% 100%, ${displayProgress}% 0)`;
    const behindDiv = document.getElementById("greet-behind-div");
    behindDiv.style.clipPath = `polygon(100% 0, 100% 100%, ${
      displayProgress - tiltPercent
    }% 100%, ${displayProgress}% 0)`;
  }

  // switch greet container and contact heading
  const middleSectionTop = document
    .getElementById("body-middle-section")
...
const toolTargeter = {
  ...
  setFrameSize: function () {
    toolTargeter.frame.style.width =
      toolTargeter.targetedIcon.clientWidth * 1.2 + "px";
    toolTargeter.frame.style.height = toolTargeter.frame.style.width;
  },
  setFramePosition: function () {
    const centerX =
      toolTargeter.targetedIcon.offsetLeft +
      toolTargeter.targetedIcon.offsetWidth / 2 -
      toolTargeter.frame.offsetWidth / 2;
    const centerY =
      toolTargeter.targetedIcon.offsetTop +
      toolTargeter.targetedIcon.offsetHeight / 2 -
      toolTargeter.frame.offsetHeight / 2;
...
function getRandomToolIcon() {
  return TOOLS_LIST[Math.floor(Math.random() * TOOLS_LIST.length)].iconElement;
}

let hoveredProjectIndex = null;
// create other-projects items from data
const otherProjectsContainerEle = document.getElementById(
...
const OPImageContainer = document.getElementById(
...
function updateOtherProjectsImage() {
  ...
  const maxX =
    projectEle.offsetLeft +
    projectEle.offsetWidth -
    OPImageContainer.offsetWidth;
  const positionX = Math.min(
    Math.max(window.mouseX - OPImageContainer.offsetWidth / 2, minX),
    maxX
  );

  const middleX =
    projectEle.offsetLeft +
    projectEle.offsetWidth / 2 -
    OPImageContainer.offsetWidth / 2;
  const imgTranslatedX =
    ((OPImage.offsetWidth - OPImage.offsetHeight) * (middleX - positionX)) /
    (middleX - minX) /
    2;
