...
function generateParticles(hex1, hex2) {
  ...
  return infoA.map((a, i) => {
    ...
    pc.midpoint = [
      (pc.pointA[0] + pc.pointB[0]) / 2,
      (pc.pointA[1] + pc.pointB[1]) / 2,
    ];
    pc.startAngle =
      90 - atan2(pc.pointA[1] - pc.midpoint[1], pc.pointA[0] - pc.midpoint[0]);
    pc.radius = dist(
      pc.midpoint[0],
      pc.midpoint[1],
      pc.pointA[0],
...
function hexTouched(hex) {
  ...
  while (true) {
    ...
    if (
      dist(
        hex.renderPosition[0],
        hex.renderPosition[1],
        randomHex.renderPosition[0],
        randomHex.renderPosition[1]
      ) /
        TILE_SCALE >
      5
    )
...
function drawTriangle(rPos, r) {
  ...
  triangle(
    ...
    (-SCALED_SQRT * 2) / 3, // top
    HALF_TILE_SCALE,
    SCALED_SQRT / 3, // bottom right
    -HALF_TILE_SCALE,
    SCALED_SQRT / 3 // bottom left
  );
  pop();
}
...
function draw() {
  ...
  for (let y = 0; y < mostY; y++) {
    for (let x = 0; x < mostX; x++) {
      ...
      if (
        isTouching &&
        !hex.isInTransition &&
        dist(hex.renderPosition[0], hex.renderPosition[1], mouseX, mouseY) <
          TILE_SCALE * 0.85
      ) {
        isTouching = false;
        resetAutoClick();
  ...
  if (curtainOpacity < 1) {
    fill(lerpColor(color2, color(0, 0), curtainOpacity));
    noStroke();
